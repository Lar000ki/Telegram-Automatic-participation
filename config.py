...
CHANNELS = ["channel1", "channel2"] # -Каналы для отслеживания розыгрышей (например, razdacha_TelegramPremium или tgpremiumdrops без символа '@', перечислять в кавычках через запятую: "razdacha_TelegramPremium", "tgpremiumdrops"). Значение 'None' означает, что бот будет получать сообщения о розыгрышах в любом канале (при этом значении вы также можете отправлять ему сообщение о розыгрыше сами)
